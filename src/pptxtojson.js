...
function getFileTypeName(type, name) {
...
async function getTheme(zip) {
  ...
  if (relationshipArray.constructor === Array) {
    for (const relationshipItem of relationshipArray) {
      if (
        relationshipItem["attrs"]["Type"] ===
        getFileTypeName(relationshipItem["attrs"]["Type"], "theme")
  ...
  const themeColors = [];
  const clrScheme = getTextByPathList(themeContent, [
    "a:theme",
    "a:themeElements",
    "a:clrScheme",
  ]);
...
async function processSingleSlide(
  ...
  for (const relationshipArrayItem of relationshipArray) {
    if (
      relationshipArrayItem["attrs"]["Type"] ==
      getFileTypeName(relationshipArrayItem["attrs"]["Type"], "media")
    ) {
      nullTarget = relationshipArrayItem;
      console.log(nullTarget);
    }
    switch (relationshipArrayItem["attrs"]["Type"]) {
      case getFileTypeName(
        relationshipArrayItem["attrs"]["Type"],
        "slideLayout"
      ):
      ...
      case getFileTypeName(
        ...
        break;
      case getFileTypeName(relationshipArrayItem["attrs"]["Type"], "image"):
      case getFileTypeName(relationshipArrayItem["attrs"]["Type"], "chart"):
      case getFileTypeName(relationshipArrayItem["attrs"]["Type"], "hyperlink"):
  ...
  for (const relationshipArrayItem of relationshipArray) {
    switch (relationshipArrayItem["attrs"]["Type"]) {
      case getFileTypeName(relationshipArrayItem["attrs"]["Type"], "theme"):
...
async function processGraphicFrameNode(node, warpObj, source) {
  ...
  switch (graphicTypeUri) {
    case getFileTypeName(graphicTypeUri, "table"):
      // case "http://schemas.openxmlformats.org/drawingml/2006/table":
      result = await genTable(node, warpObj);
      break;
    case getFileTypeName(graphicTypeUri, "chart"):
      // case "http://schemas.openxmlformats.org/drawingml/2006/chart":
      result = await genChart(node, warpObj);
      break;
    case getFileTypeName(graphicTypeUri, "diagram"):
      // case "http://schemas.openxmlformats.org/drawingml/2006/diagram":
      result = await genDiagram(node, warpObj);
      break;
    case getFileTypeName(graphicTypeUri, "ole"):
